...
class T5ProcessorNoMood:
    # Template strings for prompt generation (genre + instrument only)
    PROMPT_TEMPLATES = (
        "{genre} with {instrument}",
        "{instrument} playing {genre}",
        "recommend a {genre} track with {instrument}",
        "{genre} music featuring {instrument}",
        "a {instrument}-based {genre} song",
        "{genre} performed on {instrument}",
        "{instrument} in {genre} style",
        "create {genre} using {instrument}",
        "{genre} track with {instrument} sounds",
        "{instrument} rendition of {genre}"
    )
    ...
    def __init__(self, model_name="t5-base"):
        ...
        self.data_path = Path(self.output_dir) / "data/dataset_with_semantic_ids_nomood.csv"
        self.train_path = Path(self.output_dir) / "data/train_nomood.csv"
        self.val_path = Path(self.output_dir) / "data/val_nomood.csv"
        self.test_path = Path(self.output_dir) / "data/test_nomood.csv"
        self.model_path = Path(self.current_dir) / "models/t5_nomood_model"
        self.model_name = model_name

    def create_prompt(self, row, random_template=True):
        """
        Create a prompt from genre and instrument tags only.
        
        Args:
            row: DataFrame row with 'genre' and 'instrument' columns
            random_template: If True, randomly select template. If False, use first template.
        
        Returns:
            Formatted prompt string
        """
        genre = row.genre if not pd.isna(row.genre) else ""
        instrument = row.instrument if not pd.isna(row.instrument) else ""
        
        if not genre or not instrument:
            return None  # Both tags required
        
        if random_template:
            template = random.choice(self.PROMPT_TEMPLATES)
        else:
            template = self.PROMPT_TEMPLATES[0]  # Default: "{genre} with {instrument}"
        
        return template.format(genre=genre, instrument=instrument)
    
    def prepare_data(self, df_ids=None, csv_path=None, upsample_threshold=16, max_replication=6):
        """
        Create prompt-target pairs with varied templates for training.
        
        Args:
            df_ids: DataFrame with semantic IDs
            csv_path: Path to CSV file (alternative to df_ids)
            upsample_threshold: Threshold for upsampling rare IDs
            max_replication: Maximum number of replications for upsampling
        
        Returns:
            train_df, val_df, test_df
        """
        # Create prompt-target pairs
        print("Creating prompt-target pairs (genre + instrument)")
        ...
        for row in df.itertuples(index=False):
            ...
            if target_count < upsample_threshold:
                ...
                for _ in range(sample_count):
                    prompt = self.create_prompt(row, random_template=True)  # Varied templates
                    if prompt:
                        all_data.append({
                            'prompt': prompt,
                            'target': target
        ...
        train_df.to_csv(data_dir / "train_nomood.csv", index=False)
        val_df.to_csv(data_dir / "val_nomood.csv", index=False)
        test_df.to_csv(data_dir / "test_nomood.csv", index=False)

        print(f"train: {len(train_df)}, val: {len(val_df)}, test: {len(test_df)}")

        return train_df, val_df, test_df
    
    def train_t5(self, train_df=None, val_df=None, train_path=None, val_path=None, output_dir=None):
        """
        ...
        Args:
            ...
            output_dir: Directory to save model (default: self.model_path)
        Returns:
        ...
        # Add tokens for semantic IDs (<000> to <031>)
        new_tokens = [f"<{i:03d}>" for i in range(32)]
    ...
    def evaluate_t5(self,
                    ...
                    model_path=None, test_path=None, k=10):
        """
        Evaluate the T5 model using Hits@K metric
        Args:
            tokenizer: Trained T5 tokenizer
            model: Trained T5 model
        ...
        for batch_idx, batch in enumerate(tqdm(test_loader)):
            ...
            # Beam search to generate top k candidates
    ...
    def visualize_umap(self, 
                       tokenizer=None, model=None, test_df=None,
                       model_path=None, test_path=None, output_path=None):
        """
        ...
        if output_path is None:
            output_path = Path(self.output_dir) / "figures/umap_nomood.html"
        ...
        for i in tqdm(range(len(test_dataset)), desc="Generating embeddings"):
            data = test_dataset[i]
        ...
        fig = px.scatter_3d(
            ...
            hover_data=["prompt"],
            title="T5 Encoder UMAP Visualization (Genre + Instrument)"
        )
