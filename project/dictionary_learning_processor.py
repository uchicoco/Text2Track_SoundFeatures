...
class DictionaryLearningProcessor:
    def __init__(self, n_nonzero_coefs=2, n_dict_components=128, max_iter=100):
        """
        Args:
    ...
    def run_sparse_coding_minibatch(self, feat_pca,
        ...
        for i in range(n_samples):
            ...
            for k, idx in enumerate(top_indices):
                ...
                # assign token based on the paper
                # positive coef: <index> -> index
                # negative coef: <-index> -> index + n_dict_components
                if sign >= 0:
                    codes[i, k] = idx
                else:
                    codes[i, k] = idx + n_dict_components
